		...
		<div class="pmpro_checkout-fields">
			...
			<div id="shipping-fields">
				<div class="pmpro_checkout-field pmpro_checkout-field-sfirstname">
					...
					<?php
					if ( PMPRO_SHIPPING_SHOW_REQUIRED ) {
					?>
...
function pmproship_pmpro_memberslist_extra_cols_body( $theuser ) {
	...
	<td>
		...
		<?php if ( ! empty( $theuser->pmpro_saddress1 ) ) { ?>
			...
			<?php
			if ( ! empty( $theuser->pmpro_saddress2 ) ) {
				echo $theuser->pmpro_saddress2 . '<br />';
			}
			?>
			<?php if ( $theuser->pmpro_scity && $theuser->pmpro_sstate ) { ?>
				<?php echo $theuser->pmpro_scity; ?>, <?php echo $theuser->pmpro_sstate; ?><?php echo $theuser->pmpro_szipcode; ?>
							<?php
							if ( ! empty( $theuser->pmpro_scountry ) ) {
								echo $theuser->pmpro_scountry; }
					?>
					<br/>
			<?php } ?>
		<?php } ?>
		<?php
		if ( ! empty( $theuser->pmpro_sphone ) ) {
...
function pmproship_plugin_row_meta( $links, $file ) {
	if ( strpos( $file, 'pmpro-shipping.php' ) !== false ) {
		$new_links = array(
			'<a href="' . esc_url( '//www.paidmembershipspro.com/add-ons/shipping-address-membership-checkout/' ) . '" title="' . esc_attr( __( 'View Documentation', 'pmpro' ) ) . '">' . __( 'Docs', 'pmpro' ) . '</a>',
			'<a href="' . esc_url( '//paidmembershipspro.com/support/' ) . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pmpro' ) ) . '">' . __( 'Support', 'pmpro' ) . '</a>',
		);
